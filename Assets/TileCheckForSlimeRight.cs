...
public class TileCheckForSlimeRight : MonoBehaviour
{
    Animator anim;
    Bounds bounds;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < 8; i++) {
            if (Physics2D.Raycast(rayObject[i].transform.position, Vector2.right, 4f, 1 << 9)) {
                anim.SetBool("hasBeenSlimed", true);
                Debug.Log("true");
            }
            
            hitPlayer = Physics2D.Raycast(rayObject[i].transform.position, Vector2.right, 4f, 1 << 9);
            Debug.DrawRay(rayObject[i].transform.position , Vector2.right, Color.magenta);
            print(hitPlayer);
        }
       if (hitPlayer)
       {
